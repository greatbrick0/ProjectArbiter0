...
public class TutorialTextManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (PlayerPrefs.HasKey("shoot")) tutorialTexts[1].text = "[" + ((KeyCode)PlayerPrefs.GetInt("forward")).ToString().ToUpper() + "] to shoot";
        else tutorialTexts[1].text = "[M1] to shoot";

        if (PlayerPrefs.HasKey("reload")) tutorialTexts[2].text = tutorialTexts[1].text + "\n[" + ((KeyCode)PlayerPrefs.GetInt("reload")).ToString().ToUpper() + "] to reload";
        else tutorialTexts[2].text = tutorialTexts[1].text + "\n[R] to reload";
